	...
	$(".jquiz li ul li").click(function(){
		...
		if (!($(this).parent("ul").hasClass("answered"))) {
			...
			if (!($(this).hasClass("correct"))) {
				...
				$(this).parent().parent().children("div").prepend('<p class="incorrect"><strong>INCORRECT</strong></p>');
			...
			if ($(this).hasClass("correct")) {
				...
				// animate explanation & add styling depending on answer
				$(this).parent().parent().children("div").prepend('<p class="correcto"><strong>CORRECT</strong></p>');
				$(this).parent().parent().children("div").removeClass("hidden");
...
$(document).ready(function() {
	...
	var count = 0;
	var howmanyquestions = $("#test > li").length;
	
	//the function for a clicked item
	$("#test li ul li").click(function(){
		...
		if (!($(this).parent("ul").hasClass("answered"))) {
			...
			if ($('ul.answered').length == howmanyquestions) {
				...
				$('.quiztotal').html('I took the COMM 1010 quiz and got a '+count+' out of '+howmanyquestions+'.');
			}

			

			var totalScore = $(".quiztotal").text();
			$("#Message").append(totalScore);
